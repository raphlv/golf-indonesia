...
@section('title', 'Tambah Profil Pemain')
...
<div class="max-w-3xl mx-auto px-4 py-8 space-y-8">
    ...
    <div class="border-b border-slate-200 pb-4">
        ...
        <h1 class="text-3xl font-extrabold text-slate-800">Tambah <span class="text-golf-orange">Pegolf Baru</span></h1>
        <p class="text-sm text-slate-400 mt-1">Daftarkan profil baru untuk pegolf profesional yang bersaing di turnamen Golf Indonesia.</p>
    </div>
        ...
        <form action="{{ route('admin.players.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="country" id="country" required value="{{ old('country', 'Indonesia') }}"
                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 text-sm focus:outline-none focus:border-golf-orange"
                ...
                <div>
                    ...
                    <select name="hand" id="hand" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 text-sm focus:outline-none focus:border-golf-orange">
                        <option value="Right" {{ old('hand') === 'Right' ? 'selected' : '' }}>Kanan (Right-Handed)</option>
                        <option value="Left" {{ old('hand') === 'Left' ? 'selected' : '' }}>Kiri (Left-Handed)</option>
                    </select>
                </div>

                <!-- Photo (Optional) -->
                <div class="sm:col-span-2">
                    <label for="photo_file" class="block text-xs font-semibold text-slate-650 uppercase tracking-wider mb-2">Foto Profil (Opsional)</label>
                    <input type="file" name="photo_file" id="photo_file" accept="image/*"
                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-800 text-sm focus:outline-none focus:border-golf-orange">
                    <span class="block text-[10px] text-slate-400 mt-1">Format: JPG, JPEG, PNG (Maks 2MB)</span>
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="bio" id="bio" rows="4"
                        ...
                        placeholder="Tuliskan biografi singkat mengenai pemain, karir, prestasi dll..."></textarea>
                </div>
            </div>
